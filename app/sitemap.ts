...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const fixed = ["", "/products", "/custom-lighting", "/about", "/contact", "/blog", "/shop", "/information"].map((path) => ({ url: `${base}${path}`, lastModified: new Date(), changeFrequency: "weekly" as const, priority: path === "" ? 1 : 0.8 }));
